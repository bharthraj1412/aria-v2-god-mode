...
function providerEnvName(config, provider) {
...
function gatewayAuthHeaders(config) {
...
function normalizeDoctorFailTargets(input) {
...
function loadRawConfigObject() {
    (0, config_1.ensureConfigFile)();
    const text = fs.readFileSync(config_1.CONFIG_PATH, 'utf8');
    const parsed = yaml_1.default.parse(text);
    return parsed && typeof parsed === 'object' ? parsed : {};
}
function writeRawConfigObject(configObject) {
    fs.writeFileSync(config_1.CONFIG_PATH, yaml_1.default.stringify(configObject), 'utf8');
}
...
async function run() {
    await (0, yargs_1.default)((0, helpers_1.hideBin)(process.argv))
        ...
        .strict()
        .demandCommand(1)
        .command('onboard', 'Create local runtime config if missing', () => undefined, async () => {
        const result = (0, config_1.ensureConfigFile)();
        if (result.created) {
        ...
        .command('gateway', 'Start the OpenClaw runtime gateway', () => undefined, async () => {
        try {
            const runtime = await (0, gateway_1.startGateway)();
        ...
        .command('doctor', 'Run one-shot runtime diagnostics (config, auth, env, port, gateway reachability)', (command) => command
        .option('json', {
        type: 'boolean',
        ...
        const report = {
            configPath: config_1.CONFIG_PATH,
            gateway: {
        ...
        if (report.failOn.includes('gateway') && !report.gatewayReachability.ok) {
            report.failures.push(`gateway unreachable${report.gatewayReachability.status ? ` (HTTP ${report.gatewayReachability.status})` : ''}`);
        }
        ...
        if (report.failOn.includes('rate-limit-policy')) {
            ...
            if (rateLimit.requestsPerMinute < report.policyThresholds.minRateLimitRpm) {
                report.failures.push(`rate-limit policy requires requestsPerMinute >= ${report.policyThresholds.minRateLimitRpm} (found ${rateLimit.requestsPerMinute})`);
            }
            if (rateLimit.burst > report.policyThresholds.maxRateLimitBurst) {
                report.failures.push(`rate-limit policy requires burst <= ${report.policyThresholds.maxRateLimitBurst} (found ${rateLimit.burst})`);
            }
        }
            ...
            if (!tokenUpdatedAt) {
                ...
                if (Number.isNaN(updatedMs)) {
                    ...
                    if (ageDays > report.policyThresholds.maxTokenAgeDays) {
                        report.failures.push(`token age policy requires token age <= ${report.policyThresholds.maxTokenAgeDays} days (found ${ageDays.toFixed(1)} days)`);
                    }
                }
            }
        }
        if (argv.json) {
            ...
            console.log(`Auth: tokenConfigured=${report.gateway.tokenConfigured}, trustProxy=${report.gateway.trustProxy}, enforceLoopbackToken=${report.gateway.enforceLoopbackToken}`);
            ...
            console.log(`Gateway reachable: ${report.gatewayReachability.ok ? 'yes' : 'no'}${report.gatewayReachability.status ? ` (HTTP ${report.gatewayReachability.status})` : ''}`);
            ...
            if (report.failOn.includes('rate-limit-policy')) {
                console.log(`Rate-limit thresholds: minRateLimitRpm=${report.policyThresholds.minRateLimitRpm}, maxRateLimitBurst=${report.policyThresholds.maxRateLimitBurst}`);
            }
            if (report.failOn.includes('token-age-policy')) {
        ...
        .command('harden', 'Apply secure baseline config for auth and rate limiting', (command) => command
        .option('token', {
        type: 'string',
        ...
        const existingToken = typeof gateway.token === 'string' && gateway.token.trim().length > 0
            ? gateway.token.trim()
            : undefined;
        const token = requestedToken || existingToken || (0, node_crypto_1.randomUUID)();
        ...
        const applied = {
            ...
            gateway: {
                ...
                trustProxy: typeof nextGateway.trustProxy === 'boolean' ? nextGateway.trustProxy : false,
                enforceLoopbackToken: true,
            },
            rateLimit: {
                enabled: true,
        ...
        .command('rotate-token', 'Rotate gateway token in config', (command) => command
        .option('token', {
        type: 'string',
        ...
        const gateway = (raw.gateway && typeof raw.gateway === 'object'
            ? raw.gateway
            : {});
        const nextToken = argv.token?.trim() || (0, node_crypto_1.randomUUID)();
        const nextGateway = {
            ...gateway,
            token: nextToken,
            tokenUpdatedAt: new Date().toISOString(),
        ...
        const result = {
            ...
            tokenUpdatedAt: nextGateway.tokenUpdatedAt,
            enforceLoopbackToken: typeof nextGateway.enforceLoopbackToken === 'boolean'
        ...
        .command('configure', 'Validate provider API key environment setup', () => undefined, async () => {
        const config = (0, config_1.loadRuntimeConfig)();
        const required = [
            config.providers.openai.apiKeyEnv,
            config.providers.anthropic.apiKeyEnv,
        ...
        .command('status', 'Get runtime status from the local gateway', () => undefined, async () => {
        const config = (0, config_1.loadRuntimeConfig)();
        ...
        .command('providers', 'Check provider configuration and optional live generation', (command) => command.option('live', {
        type: 'boolean',
        ...
        try {
            const result = await (0, providers_1.generateWithFailover)(config, {
        ...
        .command('skills', 'List integrated skill catalog entries', {
        source: {
            type: 'string',
        ...
        const records = (0, skills_1.listSkills)({
            source: argv.source,
            search: argv.search,
            tag: argv.tag,
        ...
        const sources = (0, skills_1.listSkillSources)();
        const categories = (0, skills_1.listSkillCategories)({
            source: argv.source,
            search: argv.search,
            tag: argv.tag,
            capability: argv.capability,
        ...
        const summary = (0, skills_1.summarizeSkills)({
            source: argv.source,
            search: argv.search,
            tag: argv.tag,
            capability: argv.capability,
        ...
        if (argv.sources || argv.categories) {
            ...
            console.log('');
            if (argv.sources) {
                console.log(`Sources: ${listing.sources.join(', ')}`);
            }
            if (argv.categories) {
                console.log(`Categories: ${listing.categories.join(', ')}`);
            }
            if (argv.exportPath) {
        ...
        if (argv.summary) {
            ...
            console.log(`Top capabilities: ${summary.capabilities.slice(0, 5).map(item => `${item.value} (${item.count})`).join(', ') || 'none'}`);
            if (argv.exportPath) {
                console.log(`Exported: ${path.resolve(process.cwd(), argv.exportPath)}`);
            }
            return;
        ...
        .command('events', 'Stream websocket lifecycle events from the local gateway', (command) => command
        .option('timeout', {
        type: 'number',
        ...
        .command('chat <message>', 'Send a single-turn message through the local gateway', (command) => command
        .positional('message', {
        type: 'string',
        demandOption: true,
    })
